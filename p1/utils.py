...
def process_video(path: str, frames_processor: callable, nframes: int, fpargs: dict, verbose: bool = True, fps: int = None,
    ...
    while True:
        ...
        frames.append(cv.cvtColor(frame, cv.COLOR_BGR2GRAY).astype(np.int16))
        frame = frames_processor(frames, nframes, **fpargs)
...
def show_image(img: np.array, ax: plt.Subplot = None, title: str = '', cmap: str = 'gray'):
    if ax:
        ax.set_title(title)
        ax.set_yticklabels([])
        ax.set_xticklabels([])
